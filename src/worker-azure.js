...
var worker_azure_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      let processedText = escapeXml(text).replace(/,/g, ',<break time="100ms"/>').replace(/\./g, '.<break time="200ms"/>').replace(/;/g, ';<break time="120ms"/>').replace(/:/g, ':<break time="100ms"/>').replace(/\?/g, '?<break time="250ms"/>').replace(/!/g, '!<break time="250ms"/>');
