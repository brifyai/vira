...
DECLARE
    ...
    v_blocks_seconds integer := 0;
    v_ads_seconds integer := 0;
BEGIN
    ...
    IF COALESCE(v_ads_seconds, 0) = 0 AND COALESCE(v_blocks_seconds, 0) = 0 THEN
        v_blocks_seconds := v_total_seconds;
        v_ads_seconds := 0;
    END IF;

    IF v_blocks_seconds > 0 THEN
        v_charge_minutes := GREATEST(1, CEIL(v_blocks_seconds::numeric / 60.0)::integer);
    ELSE
        v_charge_minutes := 0;
    END IF;
    ...
    IF v_owner_role <> 'super_admin' AND NOT v_already_charged AND v_charge_minutes > 0 THEN
        INSERT INTO public.audio_minute_usage_events (
            ...
            jsonb_build_object(
                ...
                'blocks_duration_seconds', v_blocks_seconds,
                'ad_duration_seconds', v_ads_seconds,
                'chargeable_duration_seconds', v_blocks_seconds,
                'charge_basis', 'blocks_excluding_ads'
            )
        );
